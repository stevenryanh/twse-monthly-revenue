...
public class RankQuotesHandlerTests
{
    ...
    public async Task 有結果_應映射為DTO並保留報酬指標()
    {
        ...
        repo.Setup(r => r.RankAsync(null, null, "return", It.IsAny<int>(), It.IsAny<decimal?>(), It.IsAny<string?>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new[] { ranking });
        var handler = new RankQuotesHandler(repo.Object, NullLogger<RankQuotesHandler>.Instance);

        var result = await handler.Handle(new RankQuotesQuery(null, null, "return", 30, null, "desc"), CancellationToken.None);
    ...
    {
        var repo = new Mock<IQuoteRepository>();
        repo.Setup(r => r.RankAsync(It.IsAny<string?>(), It.IsAny<string?>(), It.IsAny<string?>(), It.IsAny<int>(), It.IsAny<decimal?>(), It.IsAny<string?>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(Array.Empty<QuoteRanking>());
        var handler = new RankQuotesHandler(repo.Object, NullLogger<RankQuotesHandler>.Instance);

        await handler.Handle(new RankQuotesQuery(null, null, null, 0, null, null), CancellationToken.None);

        repo.Verify(r => r.RankAsync(null, null, null, 30, null, null, It.IsAny<CancellationToken>()), Times.Once);
    }
}
