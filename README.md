...
# 3b.（買賣投報排行用）匯入每日股價（STOCK_DAY）
python3 scripts/import-quotes.py          # 預設 0050（ETF）+ 成分股
python3 scripts/import-quotes.py pool     # 0050∪0056∪00878∪00713 聯集（每日排程選股範圍）
# ETF 成分股快照見 scripts/etf_pools.py（0050/0056/00878/00713，共用免漂移，可自行更新）
...
`scripts/daily-fetch.sh` 會自動確保 DB + API 起來，再 upsert 最新營收與股價（不清舊資料）。
以 macOS `launchd` 排程每天 18:00 執行：

```bash
cp scripts/com.twse.daily-fetch.plist ~/Library/LaunchAgents/
launchctl load ~/Library/LaunchAgents/com.twse.daily-fetch.plist   # 啟用
launchctl start com.twse.daily-fetch                               # 立即測試一次
launchctl unload ~/Library/LaunchAgents/com.twse.daily-fetch.plist # 停用
# 紀錄：logs/daily-fetch.log
```

> 排程跑在本機，**筆電需醒著、Docker 需開著**的當下才會抓到（睡眠時不會補跑）。改時間請編輯 plist 的 `Hour`/`Minute`。
